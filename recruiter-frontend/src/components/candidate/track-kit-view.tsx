...
export function TrackKitView({
  ...
  const liveRoundSheets = trackSheets.filter((s) => (s.round ?? 1) === liveRound);
  ...
  const canWriteSheet = mySheetRead
    ...
    : (canWrite && liveRoundSheets.length === 0 && !mineElsewhere);
  ...
  return (
    <section className="space-y-3">
      ...
      {canWrite && liveRoundSheets.length > 1 && (
        <FeedbackTable questions={draft} sheets={trackSheets} interviewRound={liveRound} />
      )}
